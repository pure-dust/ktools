...
export function encodeKey(code: string) {
  let rust_key: string
...
class HotkeyManager extends Emitter {
  ...
  private readonly eventsTick: Map<string, number> = new Map()
  ...
  private eventTick() {
    const tickFn = () => {
      this.events.forEach((_, key) => {
        let diff = new Date().getTime() - this.eventsTick.get(key)!
        if (diff > 500) {
    ...
    try {
      ...
      await register(value, (event) => {
        if (event.state === "Pressed") {
          ...
          this.eventsTick.set(key, new Date().getTime())
          this.emit(key, "Pressed")
        } else if (event.state === "Released") {
          this.emit(key, "Released")
          this.events.delete(key)
          this.eventsTick.delete(key)
        }
      })
      return true
    } catch (err) {
