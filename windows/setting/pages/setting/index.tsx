...
import {Button, ColorPicker, InputNumber, Modal, Select, Shortcut} from '~components/index'
...
export default function Setting() {
  ...
  const onDelete = async (type?: number) => {
    if (type === 1) {
      delete novelCache.list[current!.path]
  ...
  return (
    <>
      ...
      <Modal width={420} rootClassName={'cache-wrapper'} title={'缓存管理'} visible={visible}
      ...
      <Modal visible={deleteVisible} title={'确认删除？'} onClose={() => setDeleteVisible(false)}>
        ...
        <Button style={{marginBottom: 6}} type={'danger'} onClick={() => onDelete(2)}>删除（删除记录和源文件）</Button>
